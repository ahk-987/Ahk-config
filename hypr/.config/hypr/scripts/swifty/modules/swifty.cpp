...
Swifty::Swifty(QWidget *parent) : QWidget(parent) {
  ...
  setFixedSize(desiredWidth,
               desiredHeight); // Get the primary screen dimensions
...
void Swifty::cleanupCache() {
  ...
  QDir picturesDir(QString::fromStdString(DIRECTORY));
...
void Swifty::loadWallpapers() {
  ...
  if (!picturesDir.exists()) {
    picturesDir.mkpath(
        "."); // Creates the directory and all needed parent directories
  }
  QStringList filters = {"*.jpg", "*.jpeg", "*.png", "*.gif"};
...
void Swifty::applyWallpaper(const QString &path) {
  ...
  QStringList currentCommand = ARGS; 

  // Replace the placeholder with the real path
  for (QString &arg : currentCommand) {
    if (arg == "WALLPATH") {
      arg = path;
    }
  }
  ...
  QProcess::startDetached(QString::fromStdString(BACKEND), currentCommand);
